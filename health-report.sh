...
if [[ "$1" == "-h" || "$1" == "--help" ]]; then
        ...
        echo "./health_report IP_Adress Option"
        exit 255
fi
...
fre_check() {
    ...
    if [[ $max_freq == *"No such file or directory"* || $min_freq == *"No such file or directory"* ]]; then
        echo "$max_freq"
        echo "$min_freq"
        echo -e "${Red}WARNING: Power management is not enabled in the OS"
        exit 1
    fi
...
turbo_mode_check()
{
        ...
        if [ "$turbo_status" == 0 ]; then
            ...
            echo -e "${Red}WARNING: Turbo Boost Disabled"
        fi
}
...
cpu_core_check()
{
        ...
        $SH $SH_HOST "lscpu | grep 'Model name' | sed 's/^[[:space:]]*//;s/[[:space:]]*$//'"
        $SH $SH_HOST "lscpu | grep 'CPU(s)' | grep -v 'NUMA node0 CPU(s)'"
        $SH $SH_HOST "lscpu | grep 'Core(s) per socket'"
}
...
disk_write_check()
{
        ...
        # get the write speed
        write_speed=$(dd if=/dev/zero of=/tmp/testfile bs=1G count=1 oflag=direct 2>&1 | tail -n 1 | awk '{print $8 " " $9 " " $10 " " $11}')
    ...
    for disk in $disks; do
        ...
        if [[ $test_result == *"Smartctl open device: /dev/$disk failed: DELL or MegaRaid controller, please try adding '-d megaraid,N'"* ]]; then
            ...
            if [[ $model == *"Seagate"* ]] || [[ $device == ST[0-9][0-9][0-9][0-9]* ]]; then
             ...
             raw_values=$(echo "$smart_test" | awk '{print $10}' | grep -v 'RAW_VALUE')
             for value in $raw_values
             do
                 if [[ $value != 0 ]]; then
                     echo -e "${Red}WARNING: Disk has errors"
                     break
                 fi
             done
            else
...
nic_err_check() {
    ...
    for interface in $interfaces; do
        if [[ $interface != "docker0" && $interface != "lo" ]]; then
            rx_err_packets=$(filter_result $interface rx_errors)
            tx_err_packets=$(filter_result $interface tx_errors)
            rx_packets=$(filter_result $interface rx_packets)
            tx_packets=$(filter_result $interface tx_packets)
            [ -z "$rx_packets" ] && rx_packets=$(filter_result $interface rx_mcast_packets)
            [ -z "$tx_packets" ] && tx_packets=$(filter_result $interface tx_mcast_packets)
            ...
            if (( $(echo "$rx_err_percent > 1" | bc -l) )) || (( $(echo "$tx_err_percent > 1" | bc -l) )); then
                ...
                echo -e $(run_ssh_cmd "ethtool -S $interface | egrep '(rx_errors|tx_errors): [1-9][0-9]*$'")
                errors_found=true
            else
...
cmos_battery ()
{
    battery_status=$($SH $SH_HOST "cat /proc/driver/rtc | grep -o 'batt_status\s*:\s*[^[:space:]]*' | cut -d ':' -f 2")
...
binary_check()
{
    binary_check=$($SH $SH_HOST "systemctl | egrep 'loopgame|gameserver|servercheck|installmanager|nodemanager|payloadproxy|seeker|jailserver'")
    echo -e "$binary_check"

    if echo "$binary_check" | grep -qv "active"; then
        echo -e "${Red}WARNING: One of the binaries is not active, please check.${NC}"
        errors_found=true
    fi
}
